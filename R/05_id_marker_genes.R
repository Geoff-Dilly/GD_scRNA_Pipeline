...
log_connection <- write_script_log(here::here("R/05_id_marker_genes.R"))
...
write.csv(all_markers, here::here("CSV_Results", "Marker_Genes_All", "All_marker_genes.csv"))
...
write.csv(top30_cell_type_markers, here::here("CSV_Results", "Marker_Genes_All", "Marker_Genes_Top30.csv"))

top10_cell_type_markers <- all_markers %>% group_by(cluster) %>% slice_max(n = 10, order_by = avg_log2FC)
write.csv(top10_cell_type_markers, here::here("CSV_Results", "Marker_Genes_All", "Marker_Genes_Top10.csv"))
...
save_plot_pdf(topmarker_dotplot, here::here("Plots/Clustering_Plots", "Top_Markers_DotPlot.pdf"), height = 8, width = 12)
...
save_plot_pdf(top2markers_dotplot, here::here("Plots/Clustering_Plots", "Top2_Markers_DotPlot.pdf"), height = 8, width = 12)
