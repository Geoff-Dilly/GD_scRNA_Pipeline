...
run_soupx_correction <- function(sample, seurat_obj, output_dir = "R_Data") {
  ...
  # Run SoupX
  soup_channel <- SoupX::SoupChannel(tod = raw_matrix, toc = filt_matrix)
  soup_channel <- SoupX::setClusters(soup_channel, setNames(cluster_labels, cell_names))
  soup_channel <- SoupX::autoEstCont(soup_channel)
  adj_matrix   <- SoupX::adjustCounts(soup_channel, roundToInt = TRUE)
