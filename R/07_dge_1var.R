...
summary_list <- foreach(cluster = clusters,
  ...
  save_plot_pdf(pca_plot_by_condition, here::here(output_dir, "plots", "DGE_Plots", "PCAs", paste0(str_replace(cluster, "_", " "), "_PCA.pdf")),
                height = 6, width = 8)

  pca_plot_by_sample <- DESeq2::plotPCA(rld, intgroup = "Sample_name", ntop = 50)
  save_plot_pdf(pca_plot_by_sample, here::here(output_dir, "plots", "DGE_Plots", "PCAs", paste0(str_replace(cluster, "_", " "), "_sample_PCA.pdf")),
                height = 6, width = 8)
  ...
  save_plot_pdf(cluster_heatmap, here::here(output_dir, "plots", "DGE_Plots", "Heatmaps", paste0(str_replace(cluster, "_", " "), "_heatmap_plot.pdf")),
                height = 6, width = 8)

  # Plot dispersion estimates
  dispersion_plot <- plotDispEsts(dds)
  save_plot_pdf(dispersion_plot, here::here(output_dir, "plots", "DGE_Plots", "Dispersion_Plots", paste0(str_replace(cluster, "_", " "), "_dispersion_plot.pdf")),
                height = 6, width = 8)
  ...
  save_plot_pdf(volc_plot, here::here(output_dir, "plots", "DGE_Plots", "Volcano_Plots", paste0(str_replace(cluster, "_", " "), "_volcano_plot.pdf")),
                height = 6, width = 8)

  # Make an MA plot for quality control
  ma_plot <- plotMA(res, ylim = c(-3,3), alpha = 0.05, main = (paste0("Cluster: ", as.character(cluster))))
  save_plot_pdf(ma_plot, here::here(output_dir, "plots", "DGE_Plots", "MA_Plots", paste0(str_replace(cluster, "_", " "), "_MA_plot.pdf")),
                height = 6, width = 8)
