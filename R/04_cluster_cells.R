...
log_connection <- write_script_log(here::here("R/04_cluster_cells.R"))
...
write.csv(cells_per_sample, here::here("CSV_Results", "Cells_per_sample.csv"))
...
save_plot_pdf(dimheatmaps_plot, here::here("Plots/Quality_Control", "DimHeatmaps.pdf"), height = 10, width = 12)

elbow_plot <- ElbowPlot(combined_seurat, ndims = 100)
save_plot_pdf(elbow_plot, here::here("Plots/Quality_Control", "ElbowPlot.pdf"), height = 4, width = 6)
...
save_plot_pdf(qc_umap_plot, here::here("Plots/Quality_Control", "QC_UMAP.pdf"), height = 4, width = 12)
...
save_plot_pdf(clustered_umap_plot, here::here("Plots/Clustering_Plots", "Clustered_UMAP.pdf"), height = 4, width = 6)

# Visualize QC metrics in each cluster
percent_mito_vln_plot <- VlnPlot(combined_seurat, features = "percent_mito", pt.size = 0)
save_plot_pdf(percent_mito_vln_plot, here::here("Plots/Quality_Control", "percentMito_Violin.pdf"), height = 4, width = 6)

nFeature_vln_plot <- VlnPlot(combined_seurat, features = "nFeature_RNA", pt.size = 0) # nolint
save_plot_pdf(nFeature_vln_plot, here::here("Plots/Quality_Control", "nFeature_Violin.pdf"), height = 4, width = 6)

nCount_vln_plot <- VlnPlot(combined_seurat, features = "nCount_RNA", pt.size = 0) # nolint
save_plot_pdf(nCount_vln_plot, here::here("Plots/Quality_Control", "nCount_Violin.pdf"), height = 4, width = 6)

doublet_score_vln_plot <- VlnPlot(combined_seurat, features = "Doublet_Score", pt.size = 0)
save_plot_pdf(doublet_score_vln_plot, here::here("Plots/Quality_Control", "Doublet_Score_Violin.pdf"), height = 4, width = 6)
